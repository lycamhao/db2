...
SELECT APPLICATION_HANDLE, MEMBER, LOCK_NAME, VARCHAR(LOCK_OBJECT_TYPE_ID,10) AS LOCK_OBJECT_TYPE_ID, VARCHAR(LOCK_OBJECT_TYPE,15) AS LOCK_OBJECT_TYPE, LOCK_MODE, LOCK_STATUS, LOCK_COUNT, LOCK_HOLD_COUNT, TBSP_ID, TAB_FILE_ID FROM TABLE(MON_GET_APP_LOCK_WAIT(NULL,-1)) WHERE TAB_FILE_ID IS NOT NULL WITH UR
...
SELECT VARCHAR(GRANTOR,15) AS GRANTOR, VARCHAR(GRANTEE,15) AS GRANTEE, VARCHAR(TABNAME,40) AS TABNAME, CONTROLAUTH, ALTERAUTH, DELETEAUTH, INSERTAUTH, SELECTAUTH, UPDATEAUTH FROM SYSCAT.TABAUTH WHERE TABNAME = 'DTABA403_TEST' WITH UR
